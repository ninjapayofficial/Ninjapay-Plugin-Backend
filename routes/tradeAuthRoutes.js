...
//           fundingProviderData.apiSecret = apiSecret;
...
// router.get("/getTradeProviders", authMiddleware, async (req, res) => {
//     const uid = req.user.uid;
//     const db = admin.firestore();
//     const userDoc = await db.collection("users").doc(uid).get();
...
//     const userData = userDoc.data();
//     const tradeProviders = userData.tradeProviders || [];
    
//     // You might want to hide secret data from the response
//     const providersToReturn = tradeProviders.map(tp => ({
//       provider: tp.provider,
//       apiKey: tp.apiKey, // In a real scenario, do NOT return the secret keys to the frontend!
//       baseUrl: tp.baseUrl
//     }));
  
//     res.status(200).json(providersToReturn);
//   });
  
//   // Remove a trade provider
//   router.post("/removeTradeProvider", authMiddleware, async (req, res) => {
//     const uid = req.user.uid;
//     const { apiKey } = req.body;
//     if (!apiKey) return res.status(400).send("API key is required to identify provider.");
...
//     const updatedProviders = tradeProviders.filter(tp => tp.apiKey !== apiKey);
    
//     await userRef.update({ tradeProviders: updatedProviders });
  
//     res.status(200).send("Trade provider removed successfully.");
//   });
  
//   // Set default trade provider
//   router.post("/setDefaultTradeProvider", authMiddleware, async (req, res) => {
//     const uid = req.user.uid;
//     const { provider } = req.body;
...
//     const userData = userDoc.data();
//     const tradeProviders = userData.tradeProviders || [];
//     const providerExists = tradeProviders.some(tp => tp.provider === provider);
  
//     if (!providerExists) {
//       return res.status(400).send("Provider not connected.");
//     }
  
//     await userRef.update({ defaultTradeProvider: provider });
//     res.status(200).send("Default trade provider set successfully.");
//   });
  
//   // Get default trade provider
//   router.get("/getDefaultTradeProvider", authMiddleware, async (req, res) => {
//     const uid = req.user.uid;
//     const db = admin.firestore();
//     const userDoc = await db.collection("users").doc(uid).get();
...
//     const userData = userDoc.data();
//     const defaultProvider = userData.defaultTradeProvider || null;
